...
describe("TransactionDetails Component", () => {
  it("renders the table with transaction data", () => {
    ...
    expect(firstRowCells[0]).toHaveTextContent("101"); // Transaction Id
    expect(firstRowCells[1]).toHaveTextContent("John Doe"); // Customer Name
    expect(firstRowCells[2]).toHaveTextContent("2025-01-05"); // Purchase Date
    expect(firstRowCells[3]).toHaveTextContent("Smartphone"); // Product Purchased
    expect(firstRowCells[4]).toHaveTextContent("599.99"); // Price

    // Check data for the second row (Laptop transaction)
    const secondRowCells = within(rows[2]).getAllByRole("cell");
    expect(secondRowCells[0]).toHaveTextContent("102"); // Transaction Id
    expect(secondRowCells[1]).toHaveTextContent("Jane Smith"); // Customer Name
    expect(secondRowCells[2]).toHaveTextContent("2025-01-10"); // Purchase Date
    expect(secondRowCells[3]).toHaveTextContent("Laptop"); // Product Purchased
    expect(secondRowCells[4]).toHaveTextContent("1200"); // Price
  });
